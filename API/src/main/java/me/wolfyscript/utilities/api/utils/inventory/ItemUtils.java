...
public class ItemUtils {
    ...
    public String convertItemStackToJson(ItemStack itemStack) {
        ...
        try {
            ...
            wolfyUtilities.getChat().sendConsoleMessage("failed to serialize itemstack to nms item");
            wolfyUtilities.getChat().sendConsoleMessage(e.toString());
            for (StackTraceElement element : e.getStackTrace()) {
                wolfyUtilities.getChat().sendConsoleMessage(element.toString());
            }
            return null;
        }
        ...
        if (itemStack != null) {
            if (itemStack.hasItemMeta()) {
                ...
                if (itemMeta.hasDisplayName()) {
                    ...
                    itemMeta.setDisplayName(me.wolfyscript.utilities.api.utils.chat.ChatColor.convert(displayName));
                }
                    ...
                    for (String row : itemMeta.getLore()) {
                        if (row.startsWith("[WU]")) {
                            ...
                            for (String newRow : wolfyUtilities.getLanguageAPI().replaceKey(row.substring("[WU!]".length()))) {
                                newLore.add(me.wolfyscript.utilities.api.utils.chat.ChatColor.convert(newRow));
                            }
                        } else {
                            newLore.add(row);
                        }
