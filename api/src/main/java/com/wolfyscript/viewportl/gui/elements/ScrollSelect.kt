 ...
 */
@Composable
fun ScrollSelect(modifier: ModifierStackBuilder = Modifier, onSubmit: (selectedItem: Int) -> Unit, icon: ItemStackSnapshot) {
    Row(
        modifier
            .defaultMinSize(
                ScrollSelectDefaults.MinWidth,
                ScrollSelectDefaults.MinHeight
            )
            .scrollSelect(onSubmit = onSubmit),
        content = {
            Icon(stack = icon)
        }
    )
