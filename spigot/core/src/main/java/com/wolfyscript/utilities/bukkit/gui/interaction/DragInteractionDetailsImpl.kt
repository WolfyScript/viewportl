...
class DragInteractionDetailsImpl(
    ...
    private val valueListeners : Multimap<Int, Consumer<ItemStack?>> = Multimaps.newListMultimap(mutableMapOf()) { mutableListOf<Consumer<ItemStack?>>() }
    ...
    override val addedStacks: Map<Int, ItemStack>
        get() = event.newItems.entries.stream().collect(
            Collectors.toUnmodifiableMap(
                { entry -> entry.key },
                { entry -> ItemStackImpl(wolfyUtils as WolfyUtilsBukkit, entry.value) }
            )
        )
}
